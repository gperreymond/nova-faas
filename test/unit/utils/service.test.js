...
describe('[unit] class Service', () => {
  ...
  it('should fail to execute a handler, because handler is undefined', async () => {
    ...
    expect(result.result['nova-faas']).to.equal(true)
    expect(result.result.package).to.equal('nova-faas')
    expect(result.result.msg).to.equal('nova-faas: service_handler_undefined')
  })
  it('should fail to execute a handler, because type is undefined', async () => {
    ...
    expect(result.result['nova-faas']).to.equal(true)
    expect(result.result.package).to.equal('nova-faas')
    expect(result.result.msg).to.equal('nova-faas: service_type_undefined')
  })
  it('should fail to execute a handler, because name is undefined', async () => {
    ...
    expect(result.result['nova-faas']).to.equal(true)
    expect(result.result.package).to.equal('nova-faas')
    expect(result.result.msg).to.equal('nova-faas: service_name_undefined')
  })
  it('should success to execute a command, and return an error', async () => {
    ...
    expect(result.result['nova-faas']).to.equal(true)
    expect(result.result.package).to.equal('nova-faas')
    expect(result.result.msg).to.equal('nova-faas: service_error')
