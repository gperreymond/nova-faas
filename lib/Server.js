...
class Server {
  ...
  start (options) {
    ...
    this.bus.on('error', error => {
      this.emit('error', cqrsError.log('internal_error', {uuid: this.uuid, type: 'Server', name: 'Server', error}))
    })
    this.bus.on('ready', () => {
      // service system
      this.bus.listen('__System.NovaServer.IsReady', async (message) => {
        console.log(message)
      })
      this.bus.publish('__System.NovaServer.IsReady', {
        uuid: this.uuid,
        type: 'server',
        starttime: this.starttime,
        services: Object.keys(this.__receivers)
      })
      this.bus.listen(`__System.NovaServer.${this.uuid}`, async (data) => {
      })
      // services discovery
